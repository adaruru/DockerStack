...
1. Named Volume: 重複掛載，不會影響初始 image，但`無法透過主機維護`。
2. Bind Mount: 會後蓋前覆蓋掉 image 初始檔案 ，適合持久化設定檔、資料庫掛載，`方便主機備份/維護/移機`。
3. Bind-backed Named Volume: 重複掛載，不會影響初始 image，解決 Named Volume 無法移機問題，但設定較繁瑣。
4. Init Script + Named Volume: Init Script Dockerfile 指定，包在 image 初始檔案，不使用 Bind Mount 避免後蓋前，`無法透過主機維護`。
